	...
	public static class DesktopFileHandle extends FileHandle {
		...
		public FileHandle child(String name) {
			if (file.getPath().length() == 0)
				return new DesktopFileHandle(new File(name), type);
		...
		public File file() {
			if (type == FileType.External)
				return new File(DesktopFileHandle.externalPath, file.getPath());

			if (type == FileType.Local)
				return new File(DesktopFileHandle.localPath, file.getPath());
