	...
	*/
	...
	public static class DefaultBehaviorTreeReader<E> extends BehaviorTreeReader {
		...
		static {
			...
			classes = new Class<?>[] { // @off - disable libgdx formatter
		...
		enum Statement {
			...
			Import("import"), Subtree("subtree"), Root("root"), TreeTask(null);
			String name;
		...
		*/
		...
		private Metadata findMetadata(Class<?> clazz) {
			...
			if (metadata == null) {
				...
				if (tca != null) {
					...
					fields = ClassReflection.getFields(clazz);
					for (Field f : fields) {
					...
					metadata = new Metadata(taskConstraint.invalid(), taskConstraint.invalid(), taskAttributes);
		...
		private static class Metadata {
			 ...
			 * Creates a 
			 * { @code  Metadata}
			 *  for a task accepting from 
			 * { @code  minChildren}
			 *  to 
			 * { @code  maxChildren}
			 *  children and the given
			 * attributes.
		...
		private static class AttrInfo {
			...
			AttrInfo(String fieldName, TaskAttribute annotation) {
				this(annotation.invalid(), fieldName, annotation.invalid());
			}
