 ...
 * A 
 * { @code  Parallel}
 *  is a special branch task that runs all children when stepped. 
 * Its actual behavior depends on its 
 * { @link orchestrator}
 *  and 
 * { @link policy}
 * .<br>
 ...
 * : the parallel task restarts or runs each child every step</li>
 * <li>
 * { @link Orchestrator#Join}
 * : child tasks will run until success or failure but will not re-run until the parallel task has succeeded or failed</li>
 * </ul>
 * The actual result of the parallel task depends on its 
 * { @link policy}
 * :
 * <ul>
 * <li>
 * { @link Policy#Sequence}
 * : the parallel task fails as soon as one child fails; if all children succeed, then the parallel
 * task succeeds. This is the default policy.</li>
 * <li>
 * { @link Policy#Selector}
 * : the parallel task succeeds as soon as one child succeeds; if all children fail, then the parallel
 * task fails.</li>
 * </ul>
...
public class Parallel<E> extends BranchTask<E> {
	...
	*/
	...
	protected Task<E> copyTo(Task<E> task) {
		...
		// no need to clone since it is immutable
	...
	/**
	 * The enumeration of the child orchestrators supported by the 
	 * { @link Parallel}
	 *  task 
	*/
	...
	*/
	public enum Policy {
		...
		* @return   { @code  Boolean.TRUE}  if parallel must succeed,  { @code  Boolean.FALSE}  if parallel must fail and  { @code  null}  if parallel must keep on running. 
		*/
