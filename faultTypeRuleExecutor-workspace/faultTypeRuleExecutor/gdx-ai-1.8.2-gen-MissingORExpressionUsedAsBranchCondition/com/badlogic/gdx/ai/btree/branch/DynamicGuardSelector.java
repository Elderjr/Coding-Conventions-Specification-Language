...
/**
 * A 
 * { @code  DynamicGuardSelector}
 *  is a branch task that executes the first child whose guard is evaluated to 
 * { @code  true}
 * . At
 * every AI cycle, the children's guards are re-evaluated, so if the guard of the running child is evaluated to 
 * { @code  false}
 * , it
 * is cancelled, and the child with the highest priority starts running. The 
 * { @code  DynamicGuardSelector}
 *  task finishes when no
 * guard is evaluated to 
 * { @code  true}
 *  (thus failing) or when its active child finishes (returning the active child's termination
 * status).
