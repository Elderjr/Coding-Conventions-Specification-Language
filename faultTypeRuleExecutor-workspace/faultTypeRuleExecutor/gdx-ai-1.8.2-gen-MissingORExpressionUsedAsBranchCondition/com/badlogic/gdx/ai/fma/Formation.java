 ...
 * A 
 * { @code  Formation}
 *  coordinates the movement of a group of characters so that they retain some group organization. Characters
 * belonging to a formation must implement the 
 * { @link FormationMember}
 *  interface. At its simplest, a formation can consist of
...
public class Formation<T extends Vector<T>> {
	...
	*/
	 ...
	 * { @code  pattern}
	 *  using a 
	 * { @link FreeSlotAssignmentStrategy}
	 *  and no motion
	 * moderator.
	...
	*/
	 ...
	 * { @code  pattern}
	 *  and 
	 * { @code  slotAssignmentStrategy}
	 *  using no motion moderator.
	* @param anchor  the anchor point of this formation, usually a  { @link Steerable} . Cannot be  { @code  null} .
	* @param pattern  the pattern of this formation
	...
	*/
	public void updateSlotAssignments() {
		...
		if (motionModerator != null)
			motionModerator.calculateDriftOffset(driftOffset, slotAssignments, pattern);
	...
	*/
	...
	private int findMemberSlot(FormationMember<T> member) {
		for (int i = 0; i < slotAssignments.size; i++) {
			if (slotAssignments.get(i).member == member)
				return i;
	...
	public void updateSlots() {
		...
		for (int i = 0; i < slotAssignments.size; i++) {
			...
			if (relativeLocPosition instanceof Vector2)
				...
				((Vector3) relativeLocPosition).mul(orientationMatrix);
