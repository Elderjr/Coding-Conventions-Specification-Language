...
public abstract class SchedulerBase<T extends SchedulerBase.SchedulableRecord> implements Scheduler {
	...
	*/
	 ...
	 *  and calculates the best phase based on the
	 ...
	 * the size of the least common multiple (LCM, see 
	 * { @link ArithmeticUtils#lcmPositive(int , int )}
	 * ) of all the frequency values
	...
	*/
	protected int calculatePhase(int frequency) {
		if (frequency > phaseCounters.size)
			phaseCounters.ensureCapacity(frequency - phaseCounters.size);

		int[]

		items = phaseCounters.items;
		// Reset counters
