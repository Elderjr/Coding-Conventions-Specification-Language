...
/**
 * A 
 * { @code  LoadBalancingScheduler}
 *  understands the time it has to run and distributes this time among the tasks that need to be
 ...
 * the scheduler, with a frequency of 
 * { @code  f}
 * , we perform a dry run of the scheduler for a fixed number of frames into the
 ...
 * multiple (LCM) of all the frequency values used in the scheduler, see 
 * { @link ArithmeticUtils#lcmPositive(int , int )}
 * . Typically,
...
public class LoadBalancingScheduler extends SchedulerBase<SchedulableRecord> {
	...
	*/
	 ...
	 * { @code  LoadBalancingScheduler}
	 * .
	* @param dryRunFrames  number of frames simulated by the dry run to calculate the phase when adding a schedulable via { @link #addWithAutomaticPhasing(Schedulable , int )}  
	*/
	 ...
	 * Adds the 
	 * { @code  schedulable}
	 *  to the list using the given 
	 * { @code  frequency}
	 *  and a phase calculated by a dry run of the
	 * scheduler.
