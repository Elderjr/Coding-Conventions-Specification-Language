...
/**
 * A 
 * { @code  ParallelSideRayConfiguration}
 *  uses two rays parallel to the direction of motion. The rays have the same length and
...
public class ParallelSideRayConfiguration<T extends Vector<T>> extends RayConfigurationBase<T> {
	 ...
	 * Creates a 
	 * { @code  ParallelSideRayConfiguration}
	 *  for the given owner where the two rays have the specified length and side
	 * offset.
	...
	*/
	...
	public Ray<T>[] updateRays() {
		...
		rays[0].end.set(owner.getLinearVelocity()).nor().scl(length);
		// later we'll add rays[0].start;
		// Update ray 1
