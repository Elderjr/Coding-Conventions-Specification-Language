 ...
 * First the 
 * { @code  Jump}
 *  behavior calculates the linear velocity required to achieve the jump. If the calculated velocity doesn't
 ...
 * { @link JumpCallback#reportAchievability(boolean )  reportAchievability}
 *  method. Also, if the jump is
 * achievable the run up phase begins and the 
 * { @code  Jump}
 *  behavior will start to produce the linear acceleration required to match
 ...
 * the callback is told to jump through the 
 * { @link JumpCallback#takeoff(float , float )  takeoff}
 *  method.
...
public class Jump<T extends Vector<T>> extends MatchVelocity<T> {
	...
	*/
	 ...
	 * returned by the 
	 * { @link GravityComponentHandler#getComponent(Vector )}
	 *  method. 
	*/
	protected T gravity;
	...
	public SteeringAcceleration<T> calculateRealSteering(SteeringAcceleration<T> steering) {
		...
		if (owner.getPosition().epsilonEquals(target.getPosition(), takeoffPositionTolerance)) {
			...
			if (owner.getLinearVelocity().epsilonEquals(target.getLinearVelocity(), takeoffVelocityTolerance)) {
				...
				if (DEBUG_ENABLED)
					GdxAI.getLogger().info("Jump", "Bad Velocity: Speed diff. = "
							+ planarVelocity.set(target.getLinearVelocity()).sub(owner.getLinearVelocity()).len()
							+ ", diff = (" + planarVelocity + ")");
	...
	*/
	private Steerable<T> calculateTarget() {
		...
		this.airborneTime = calculateAirborneTimeAndVelocity(jumpTarget.linearVelocity, jumpDescriptor,
				getActualLimiter().getMaxLinearSpeed());
	 ...
	 * { @code  outVelocity}
	 *  vector to the airborne planar velocity required to achieve the
	 * jump. If the jump is not achievable -1 is returned and the 
	 * { @code  outVelocity}
	...
	*/
	public float calculateAirborneTimeAndVelocity(T outVelocity, JumpDescriptor<T> jumpDescriptor,
		...
		if (planarVelocity.len2() < maxLinearSpeed * maxLinearSpeed) {
			...
			if (DEBUG_ENABLED)
				GdxAI.getLogger().info("Jump",
						"targetLinearVelocity = " + outVelocity + "; targetLinearSpeed = " + outVelocity.len());
	...
	*/
	...
	/**
	 * Sets the target whose velocity should be matched. Notice that this method is inherited from 
	 * { @link MatchVelocity}
	 * . Usually
	 * with 
	 * { @code  Jump}
	 *  you should never call it because a specialized internal target has already been created implicitly.
	...
	* A 
	* { @code  JumpDescriptor}
	*  contains jump information like the take-off and the landing position.
	* @param < T >  Type of vector, either 2D or 3D, implementing the  { @link Vector}  interface
	* @author  davebaol 
	*/
	public static class JumpDescriptor<T extends Vector<T>> {
		...
		*/
		 ...
		 * Creates a 
		 * { @code  JumpDescriptor}
		 *  with the given takeoff and landing positions.
		...
		*/
		 ...
		 * Sets this 
		 * { @code  JumpDescriptor}
		 *  from the given takeoff and landing positions.
		* @param takeoffPosition  the position of the takeoff pad
		* @param landingPosition  the position of the landing pad 
		*/
	...
	/**
	* A 
	* { @code  GravityComponentHandler}
	*  is aware of the axis along which the gravity acts.
	* @param < T >  Type of vector, either 2D or 3D, implementing the  { @link Vector}  interface
	* @author  davebaol 
	*/
	...
	* The 
	* { @code  JumpCallback}
	*  allows you to know whether a jump is achievable and when to jump.
	* @author  davebaol 
	*/
