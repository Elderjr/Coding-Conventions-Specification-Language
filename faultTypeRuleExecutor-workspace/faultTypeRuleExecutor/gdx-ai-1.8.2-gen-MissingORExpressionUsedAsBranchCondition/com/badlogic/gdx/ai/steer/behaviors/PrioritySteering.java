...
/**
 * The 
 * { @code  PrioritySteering}
 *  behavior iterates through the behaviors and returns the first non zero steering. It makes sense
 * since certain steering behaviors only request an acceleration in particular conditions. Unlike 
 * { @link Seek}
 *  or 
 * { @link Evade}
 * ,
 * which always produce an acceleration, 
 * { @link RaycastObstacleAvoidance}
 * , 
 * { @link CollisionAvoidance}
 * , 
 * { @link Separation}
 * ,
 * { @link Hide}
 *  and 
 * { @link Arrive}
 *  will suggest no acceleration in many cases. But when these behaviors do suggest an
 * acceleration, it is unwise to ignore it. An obstacle avoidance behavior, for example, should be honored immediately to avoid
 * the crash.
 * <p>
 * Typically the behaviors of a 
 * { @code  PrioritySteering}
 *  are arranged in groups with regular blending weights, see
 ...
 * small constant value (conventionally called 
 * { @code  epsilon}
 * ) avoids this problem. When a group is found with a result that isn't
 ...
 * Usually 
 * { @code  PrioritySteering}
 *  gives you a good compromise between speed and accuracy.
...
public class PrioritySteering<T extends Vector<T>> extends SteeringBehavior<T> {
	...
	*/
	 ...
	 * Creates a 
	 * { @code  PrioritySteering}
	 *  behavior for the specified owner. The threshold is set to 0.001.
	...
	*/
	 ...
	 * Creates a 
	 * { @code  PrioritySteering}
	 *  behavior for the specified owner and threshold.
	* @param owner  the owner of this behavior
	* @param epsilon  the threshold of the steering acceleration magnitude below which a steering behavior is considered to have given no output 
	*/
	...
	*/
	...
	/**
	 * Sets the limiter of this steering behavior. However, 
	 * { @code  PrioritySteering}
	 *  needs no limiter at all as it simply returns
