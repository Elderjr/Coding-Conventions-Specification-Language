 ...
 * <p>
 * Typically (but not necessarily) different group behaviors share the same 
 * { @code  Proximity}
 *  for a given owner. This allows you to
 ...
 * { @link Separation  separation}
 * , 
 * { @link Alignment  alignment}
 * , and 
 * { @link Cohesion  cohesion}
 * . The three behaviors are typically
 * combined through a 
 * { @link BlendedSteering  blended steering}
 * . This works okay but, because of the limited view distance of a
 * character, it's possible for an agent to become isolated from its flock. If this happens, it will just sit still and do
 * nothing. To prevent this from happening, you usually add in the 
 * { @link Wander  wander}
 *  behavior too. This way, all the agents
 ...
 * <li>Sharing a 
 * { @code  Proximity}
 *  instance among group behaviors having the same owner can save a little time determining the
 * neighbors only once from inside the 
 * { @code  findNeighbors}
 *  method. Especially, 
 * { @code  Proximity}
 *  implementation classes can use
 * { @link Timepiece#getTime()  GdxAI.getTimepiece().getTime()}
 *  to calculate neighbors only once per frame (assuming delta time is
 ...
 *  on each frame the proximity instance will be
 * calculated only the very first time, which is not what you want of course.</li>
 * <li>ideally the timepiece should be updated before the proximity is updated by the 
 * { @link #findNeighbors(ProximityCallback )}
 * method.</li>
 * </ul>
 * </li>
 ...
 * owner itself, you have to implement a callback that prevents it from being considered by returning 
 * { @code  false}
 *  from the method
 * { @link ProximityCallback#reportNeighbor(Steerable )  reportNeighbor}
 * .</li>
...
public interface Proximity<T extends Vector<T>> {
	...
	*/
	 ...
	 * { @link ProximityCallback#reportNeighbor(Steerable )  reportNeighbor}
	 *  method of the specified callback.
