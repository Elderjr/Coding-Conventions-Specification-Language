...
/**
 * { @code  Wander}
 *  behavior is designed to produce a steering acceleration that will give the impression of a random walk through
 ...
 * <ul>
 * <li>The owner seeks the target, using the 
 * { @link Seek}
 *  behavior, and performs a 
 * { @link LookWhereYouAreGoing}
 *  behavior to
 * correct its orientation.</li>
 * <li>The owner tries to face the target in each frame, using the 
 * { @link Face}
 *  behavior to align to the target, and applies full
 ...
 * time step, 
 * { @link Face}
 *  behavior should not be used (which is the default case). On the other hand, if the owner has
 * independent facing you should explicitly call 
 * { @link #setFaceEnabled(boolean )  setFaceEnabled(true)}
 *  before using Wander
 * behavior.
 * <p>
 * Note that this behavior internally calls the 
 * { @link Timepiece#getTime()  GdxAI.getTimepiece().getTime()}
 *  method to get the
 * current AI time and make the 
 * { @link wanderRate}
 *  FPS independent. This means that
 * <ul>
 * <li>if you forget to 
 * { @link Timepiece#update(float )  update the timepiece}
 *  the wander orientation won't change.</li>
...
public class Wander<T extends Vector<T>> extends Face<T> {
	...
	*/
	 ...
	 * { @link Face}
	 *  behavior or not. This should be set to 
	 * { @code  true}
	 *  when independent facing
	 * is used. 
	*/
	 ...
	 * Creates a 
	 * { @code  Wander}
	 *  behavior for the specified owner.
	...
	*/
	...
	protected SteeringAcceleration<T> calculateRealSteering(SteeringAcceleration<T> steering) {
		...
		wanderCenter.set(owner.getPosition()).mulAdd(owner.angleToVector(steering.linear, owner.getOrientation()),
				wanderOffset);
		// Calculate the target location
		// Notice that we're using steering.linear as temporary vector
		internalTargetPosition.set(wanderCenter).mulAdd(owner.angleToVector(steering.linear, targetOrientation),
				wanderRadius);
	...
	*/
	 ...
	 * additionally, if the flag 
	 * { @code  faceEnabled}
	 *  is true, it must take care of the maximum angular speed and acceleration.
	* @return  this behavior for chaining. 
	*/
	@Override
	...
	/**
	 * Sets the target to align to. Notice that this method is inherited from 
	 * { @link ReachOrientation}
	 * , but is completely useless
	 * for 
	 * { @code  Wander}
	 *  because owner's orientation is determined by the internal target, which is moving on the wander circle.
	* @return  this behavior for chaining. 
	*/
	@Override
