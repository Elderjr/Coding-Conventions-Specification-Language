...
/**
 * { @code  FollowPath}
 *  behavior produces a linear acceleration that moves the agent along the given path. First it calculates the
 * agent location based on the specified prediction time. Then it works out the position of the internal target based on the
 * location just calculated and the shape of the path. It finally uses 
 * { @link Seek  seek}
 *  behavior to move the owner towards the
 * internal target position. However, if the path is open 
 * { @link Arrive  arrive}
 *  behavior is used to approach path's extremities
 * when they are far less than the 
 * { @link FollowPath#decelerationRadius  deceleration radius}
...
public class FollowPath<T extends Vector<T>, P extends PathParam> extends Arrive<T> {
	...
	*/
	 ...
	 * { @link Arrive}
	 *  behavior to approach the end of an open path. It defaults to 
	 * { @code  true}
	 * . 
	*/
	...
	*/
	 ...
	 * { @code  FollowPath}
	 *  behavior for the specified owner and path.
	...
	*/
	 ...
	 * Creates a non-predictive 
	 * { @code  FollowPath}
	 *  behavior for the specified owner, path and path offset.
	* @param owner  the owner of this behavior
	* @param path  the path to be followed by the owner
	* @param pathOffset  the distance along the path to generate the target. Can be negative if the owner is to move along the reverse direction. 
	*/
	 ...
	 * Creates a 
	 * { @code  FollowPath}
	 *  behavior for the specified owner, path, path offset, maximum linear acceleration and prediction
	 * time.
	* @param owner  the owner of this behavior
	* @param path  the path to be followed by the owner
	* @param pathOffset  the distance along the path to generate the target. Can be negative if the owner is to move along the reverse direction.
	...
	*/
	...
	protected SteeringAcceleration<T> calculateRealSteering(SteeringAcceleration<T> steering) {
		...
		T location = (predictionTime == 0) ? // Use the current position of the owner
				owner.getPosition() : // Calculate the predicted future position of the owner. We're reusing steering.linear here.
	...
	*/
	 ...
	 * { @link Arrive}
	 *  behavior to approach the end of an open path. 
	*/
