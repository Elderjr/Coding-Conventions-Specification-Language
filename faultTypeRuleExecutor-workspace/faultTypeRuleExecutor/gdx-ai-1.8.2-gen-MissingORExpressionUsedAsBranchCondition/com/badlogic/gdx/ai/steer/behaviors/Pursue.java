...
/**
 * { @code  Pursue}
 *  behavior produces a force that steers the agent towards the evader (the target). Actually it predicts where an
...
public class Pursue<T extends Vector<T>> extends SteeringBehavior<T> {
	...
	*/
	 ...
	 * Creates a 
	 * { @code  Pursue}
	 *  behavior for the specified owner and target. Maximum prediction time defaults to 1 second.
	...
	*/
	 ...
	 * Creates a 
	 * { @code  Pursue}
	 *  behavior for the specified owner and target.
	* @param owner  the owner of this behavior
	* @param target  the target of this behavior
	* @param maxPredictionTime  the max time used to predict the target's position assuming it continues to move with its current velocity. 
	*/
	...
	/**
	 * Returns the actual linear acceleration to be applied. This method is overridden by the 
	 * { @link Evade}
	 *  behavior to invert the
	...
	*/
	...
	protected SteeringAcceleration<T> calculateRealSteering(SteeringAcceleration<T> steering) {
		...
		steering.linear.set(targetPosition).mulAdd(target.getLinearVelocity(), predictionTime).sub(owner.getPosition())
