...
/**
 * With the 
 * { @code  RaycastObstacleAvoidance}
 *  the moving agent (the owner) casts one or more rays out in the direction of its
 ...
 * <p>
 * You should use the 
 * { @link RayConfiguration}
 *  more suitable for your game environment. Some basic ray configurations are provided
 * by the framework: 
 * { @link SingleRayConfiguration}
 * , 
 * { @link ParallelSideRayConfiguration}
 *  and
 * { @link CentralRayWithWhiskersConfiguration}
 * . There are no hard and fast rules as to which configuration is better. Each has its
...
public class RaycastObstacleAvoidance<T extends Vector<T>> extends SteeringBehavior<T> {
	...
	*/
	...
	protected SteeringAcceleration<T> calculateRealSteering(SteeringAcceleration<T> steering) {
		...
		inputRays = rayConfiguration.updateRays();
		// Process rays
		...
		steering.linear.set(minOutputCollision.point)
				.mulAdd(minOutputCollision.normal, owner.getBoundingRadius() + distanceFromBoundary)
				.sub(owner.getPosition()).nor().scl(getActualLimiter().getMaxLinearAcceleration());
	...
	* @return  this behavior for chaining. 
	*/
	public RaycastObstacleAvoidance<T> setRaycastCollisionDetector(
			RaycastCollisionDetector<T> raycastCollisionDetector) {
		this.raycastCollisionDetector = raycastCollisionDetector;
		return this;
	}
