 ...
 * A 
 * { @code  LinePath}
 *  is a path for path following behaviors that is made up of a series of waypoints. Each waypoint is connected
 * to the successor with a 
 * { @link Segment}
 * .
...
public class LinePath<T extends Vector<T>> implements Path<T, LinePathParam> {
	...
	*/
	...
	public void calculateTargetPosition(T out, LinePathParam param, float targetDistance) {
		...
		out.set(desiredSegment.begin).sub(desiredSegment.end).scl(distance / desiredSegment.length)
	...
	* A 
	* { @code  Segment}
	*  connects two consecutive waypoints of a 
	* { @link LinePath}
	* .
	* @param < T >  Type of vector, either 2D or 3D, implementing the  { @link Vector}  interface
	* @author  davebaol 
