...
/**
 * { @code  Interpose}
 *  behavior produces a steering force that moves the owner to a point along the imaginary line connecting two
 ...
 * behavior. Like 
 * { @code  Pursue}
 * , the owner must estimate where the two agents are going to be located at a time 
 * { @code  t}
 *  in the
 * future. It can then steer toward that position using the 
 * { @link Arrive}
 *  behavior. But how do we know what the best value of
 * { @code  t}
 *  is to use? The answer is, we don't, so we make a calculated guess instead.
 * <p>
 ...
 * step. This point is found taking into account the 
 * { @code  interpositionRatio}
 * , a number between 0 and 1 where 0 is the position
 ...
 * locations.
 * <p>
 * Then the distance from this point is computed and the value divided by the owner's maximum speed to give the time 
 * { @code  t}
 * required to travel the distance.
 * <p>
 * Using the time 
 * { @code  t}
 * , the agents' positions are extrapolated into the future. The target position in between of these
...
public class Interpose<T extends Vector<T>> extends Arrive<T> {
	 ...
	 * Creates an 
	 * { @code  Interpose}
	 *  behavior for the specified owner and agents using the midpoint between agents as the target.
	...
	*/
	 ...
	 * Creates an 
	 * { @code  Interpose}
	 *  behavior for the specified owner and agents using the the given interposing ratio.
	* @param owner  the owner of this behavior
	* @param agentA  the first agent
	* @param agentB  the other agent
	* @param interpositionRatio  a number between 0 and 1 indicating the percentage of the distance between the 2 agents that the owner should reach, where 0 is agentA position and 1 is agentB position. 
	*/
	...
	* @param interpositionRatio  a number between 0 and 1 indicating the percentage of the distance between the 2 agents that the owner should reach. Especially, 0 is the position of agentA and 1 is the position of agentB.
	* @return  this behavior for chaining. 
	*/
	...
	protected SteeringAcceleration<T> calculateRealSteering(SteeringAcceleration<T> steering) {
		...
		float timeToTargetPosition = owner.getPosition().dst(internalTargetPosition)
