...
/**
 * { @code  FieldOfViewProximity}
 *  emulates the peripheral vision of the owner as if it had eyes. Any agents contained in the
 ...
 * <p>
 * Note that this implementation checks the AI time of the current frame through the 
 * { @link Timepiece#getTime() GdxAI.getTimepiece().getTime()}
 *  method in order to calculate neighbors only once per frame (assuming delta time is always
...
public class FieldOfViewProximity<T extends Vector<T>> extends ProximityBase<T> {
	...
	*/
	 ...
	 * Creates a 
	 * { @code  FieldOfViewProximity}
	 *  for the specified owner, agents and cone area defined by the given radius and angle
	 * in radians.
	...
	*/
	public FieldOfViewProximity(Steerable<T> owner, Iterable<? extends Steerable<T>> agents, float radius,
			float angle) {
		super(owner, agents);
		this.radius = radius;
		setAngle(angle);
