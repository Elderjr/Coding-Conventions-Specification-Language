 ...
 * A fully implemented 
 * { @link PathFinder}
 *  that can perform both interruptible and non-interruptible pathfinding.
 * <p>
 ...
 * must be numbered using sequential integers (see 
 * { @link IndexedGraph#getIndex(Object )}
 * ), so we don't need to search for a node in the
 * two lists at all. We can simply use the node index to look up its record in the array (creating it if it is missing). This
 * means that the close list is no longer needed. To know whether a node is open or closed, we use the 
 * { @link NodeRecord#category category}
 *  of the node record. This makes the search step very fast indeed (in fact, there is no search, and we can go straight
 ...
 * element with the lowest cost. However, we use a 
 * { @link BinaryHeap}
 *  for the open list in order to keep performance as high as
 * possible.
