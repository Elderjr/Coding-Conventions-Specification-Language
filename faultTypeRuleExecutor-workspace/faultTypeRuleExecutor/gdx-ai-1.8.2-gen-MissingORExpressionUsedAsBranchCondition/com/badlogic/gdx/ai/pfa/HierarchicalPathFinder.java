...
/**
 * A 
 * { @code  HierarchicalPathFinder}
 *  can find a path in an arbitrary 
 * { @link HierarchicalGraph}
 *  using the given 
 * { @link PathFinder}
 * ,
 ...
 * <p>
 * Note that the hierarchical path finder calls the 
 * { @link HierarchicalGraph#setLevel(int )}
 *  method to switch the graph into a
 * particular level. All future calls to the 
 * { @link HierarchicalGraph#getConnections(Object )  getConnections}
 *  method of the
...
public class HierarchicalPathFinder<N> implements PathFinder<N> {
	...
	public boolean searchConnectionPath(N startNode, N endNode, Heuristic<N> heuristic,
		...
		while (currentLevel >= 0) {
			...
			graph.setLevel(levelOfNodes);
			outPath.clear();
			boolean pathFound = levelPathFinder.searchConnectionPath(currentStartNode, currentEndNode, heuristic,
					outPath);
			if (!pathFound)
				return false;
	...
	public boolean search(PathFinderRequest<N> request, long timeToRun) {
		...
		if (request.statusChanged) {
			...
			levelRequestControl.lastTime = TimeUtils.nanoTime();
	...
	static class LevelPathFinderRequest<N> extends PathFinderRequest<N> {
		...
		public boolean initializeSearch(long timeToRun) {
			...
			do {
				...
				if (DEBUG)
					GdxAI.getLogger().debug(TAG, "LevelPathFinder initializeSearch");
		...
		public boolean search(PathFinder<N> pathFinder, long timeToRun) {
			if (DEBUG)
				GdxAI.getLogger().debug(TAG, "LevelPathFinder search; status: " + status);
		...
		public boolean finalizeSearch(long timeToRun) {
			...
			if (DEBUG)
				GdxAI.getLogger().debug(TAG, "LevelPathFinder finalizeSearch; status: " + status);
