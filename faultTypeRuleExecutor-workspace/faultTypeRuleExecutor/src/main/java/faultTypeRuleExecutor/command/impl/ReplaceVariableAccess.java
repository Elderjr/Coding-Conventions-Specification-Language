...
public class ReplaceVariableAccess implements InjectionAction {

	private void getAllCandidateVariables(ASTNode currentNode, ASTNode previousNode, List<VariableDeclaration> selectedNodes){
	...
	private Type getVariableType(VariableDeclaration v) {
		if(v instanceof SingleVariableDeclaration) {
			...
			if(varDecl.getVariablesContainer().getType() != null) {
	...
	private boolean hasSameType(VariableDeclaration v1, VariableDeclaration v2) {
		...
		return v1.getExtraArrayDimensions() == v2.getExtraArrayDimensions() &&
				typeV1 != null && typeV2 != null && getVariableType(v1) == getVariableType(v2);
	}
	...
	public boolean doAction(ASTNode target) {
		...
		SingleVariableAccess varAccess = (SingleVariableAccess) target;
		List<VariableDeclaration> candidates = new LinkedList<>();
		getAllCandidateVariables(varAccess, null, candidates);
		ASTNode container = (ASTNode) varAccess.eContainer();
		while(container != null && !(container instanceof AbstractVariablesContainer)) {
		...
		for(VariableDeclaration v : candidates) {
			if(varAccess.getVariable() != v && hasSameType(varAccess.getVariable(), v)) {
				SingleVariableAccess newVarAccess = JavaPackage.eINSTANCE.getJavaFactory().createSingleVariableAccess();
				newVarAccess.setVariable(v);
				ActionUtils.setValue(varAccess, newVarAccess);
				return true;
			}
		}
		return false;
