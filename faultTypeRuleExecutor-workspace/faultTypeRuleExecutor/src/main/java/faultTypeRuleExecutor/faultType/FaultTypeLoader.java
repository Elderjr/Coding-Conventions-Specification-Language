...
import faultTypeRuleExecutor.xmlReader.FaultTypeDescriptionXMLReader;
...
public final class FaultTypeLoader {
	...
	public FaultTypeDescription loadFaultTypeDescriptionNEW(String pathFile) throws Exception {
		File xmlFile = new File(pathFile);
		FaultTypeDescriptionXMLNode node = FaultTypeDescriptionXMLReader.getInstance()
				.readFaultTypeDescriptionFromXML(pathFile);
		System.out.println(node.getOcl());
		FaultTypeDescription faultTypeDescription = XMLNode2FaultTypeDescriptionParser.getInstance()
				.xmlNodeToFaultTypeDescription(node, xmlFile.getParent());
